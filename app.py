...
with st.sidebar:
    ...
    for p in ["Dashboard", "Nova negociação", "Negociações", label_aprov, "Fornecedores", "Relatórios"]:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # Monta lista de fornecedores para o selectbox
    forn_nomes = ["— Selecione —"] + [f["nome"] for f in fornecedores]
    forn_map   = {f["nome"]: f for f in fornecedores}

    # Seleção fora do form para poder atualizar o CNPJ dinamicamente
    st.subheader("Fornecedor")
    col1, col2 = st.columns(2)
    forn_selecionado = col1.selectbox("Fornecedor *", forn_nomes)
    cnpj_auto = forn_map[forn_selecionado]["cnpj"] if forn_selecionado != "— Selecione —" else ""
    col2.text_input("CNPJ", value=cnpj_auto, disabled=True)

    if forn_selecionado == "— Selecione —":
        st.info("Selecione um fornecedor acima. Se não encontrar, cadastre em **Fornecedores** no menu lateral.")
    ...
    with st.form("form_neg", clear_on_submit=True):
        ...
        if valor_total > 0:
            st.markdown(f"**Total: {brl(valor_total)}**")
    ...
    if submitted:
        erros = []
        if forn_selecionado == "— Selecione —": erros.append("Selecione um fornecedor.")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.caption("Cadastre os fornecedores para agilizar o registro de negociações")
    ...
    with st.expander("➕ Cadastrar novo fornecedor", expanded=len(fornecedores) == 0):
        with st.form("form_forn", clear_on_submit=True):
            ...
            obs_f   = c4.text_input("Observações (opcional)", placeholder="Informações adicionais")
    ...
    if not fornecedores:
        ...
        busca_f = st.text_input("🔍 Buscar fornecedor", placeholder="Nome ou CNPJ...", label_visibility="collapsed")
        ...
        for f in lista:
            with st.expander(f"**{f['nome']}** — {f.get('cnpj','—')}"):
                c1, c2 = st.columns(2)
                novo_nome    = c1.text_input("Nome",    value=f["nome"],              key=f"fn_{f['id']}")
                novo_cnpj    = c2.text_input("CNPJ",    value=f.get("cnpj",""),       key=f"fc_{f['id']}")
                c3, c4       = st.columns(2)
                novo_contato = c3.text_input("Contato", value=f.get("contato",""),    key=f"fct_{f['id']}")
                novo_obs     = c4.text_input("Obs",     value=f.get("obs",""),        key=f"fo_{f['id']}")

                cb, cd, _ = st.columns([1, 1, 4])
                if cb.button("💾 Salvar", key=f"fs_{f['id']}", type="primary"):
                    update_fornecedor(f["id"], {"nome": novo_nome, "cnpj": novo_cnpj,
                                                "contato": novo_contato, "obs": novo_obs})
                    reload()
                    st.success("Atualizado!")
                    st.rerun()
                if cd.button("🗑 Excluir", key=f"fd_{f['id']}"):
                    delete_fornecedor(f["id"])
                    reload()
                    st.rerun()
